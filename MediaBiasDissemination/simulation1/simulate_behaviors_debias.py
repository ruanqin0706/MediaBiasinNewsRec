...
def generate_pseudo_input(original_behaviors_path,
    ...
    # generate current and previous news based on pseudo_step
    news_info_df = news_df[news_df['step'] <= pseudo_step]
    if apply_news_path:
        assert apply_mode in ("apply_hist", "apply_candidate", "apply_both")
        replaced_news_df = pd.read_csv(apply_news_path, header=None, sep="\t")
        replaced_nid2title = dict(zip(replaced_news_df[0], replaced_news_df[3]))

        def replace_title_in_both(title, nid, replaced_nid2title):
            return replaced_nid2title[nid]

        def replace_title_in_candidate(title, nid, candidate_nid_list, replaced_nid2title):
            if nid in candidate_nid_list:
                return replaced_nid2title[nid]
            else:
                return title

        def replace_title_in_hist(title, nid, candidate_nid_list, replaced_nid2title):
            if nid in candidate_nid_list:
                return title
            else:
                return replaced_nid2title[nid]

        if apply_mode == "apply_hist":
            news_info_df["title"] = news_info_df.apply(lambda row: replace_title_in_hist(
                title=row['title'],
                nid=row['nid'],
                candidate_nid_list=candidate_nid_list,
                replaced_nid2title=replaced_nid2title), axis=1)
        elif apply_mode == "apply_candidate":
            news_info_df["title"] = news_info_df.apply(lambda row: replace_title_in_candidate(
                title=row['title'],
                nid=row['nid'],
                candidate_nid_list=candidate_nid_list,
                replaced_nid2title=replaced_nid2title), axis=1)
        else:
            news_info_df["title"] = news_info_df.apply(lambda row: replace_title_in_both(
                title=row['title'],
                nid=row['nid'],
                replaced_nid2title=replaced_nid2title), axis=1)
...
def parse_args():
    ...
    parser.add_argument("--original_behaviors_path", type=str,
                        default="user_groups.tsv")
    parser.add_argument("--original_news_path", type=str,
                        default="news.tsv")
    parser.add_argument("--nid2bias_path", type=str,
                        default="nid2bias_prob.pkl")
    ...
    parser.add_argument("--apply_mode", type=str, choices=["apply_hist", "apply_candidate", "apply_both"],
                        default="apply_hist")
    parser.add_argument("--apply_news_path", type=str, default="")
...
if __name__ == '__main__':
    ...
    generate_pseudo_input(original_behaviors_path=args.original_behaviors_path,
                          ...
                          apply_news_path=args.apply_news_path
                          )
